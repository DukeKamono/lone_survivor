...
impl Enemy {
    pub fn new() -> Enemy {
        ...
        match side {
            s if s == 0 => {
                ypos = 0.0;
                xpos = rng.gen_range(0, 800) as f32;
            }
            s if s == 1 => {
                ypos = 600.0;
                xpos = rng.gen_range(0, 800) as f32;
            ...
            s if s == 2 => {
                ypos = rng.gen_range(0, 600) as f32;
                xpos = 0.0;
            }
            s if s == 3 => {
                ypos = rng.gen_range(0, 600) as f32;
    ...
    pub fn despawn(&mut self, player: &Player) {
        if player.hitbox.overlaps(&self.hitbox) && self.hp != 0 {
            self.hp = 0;
        }
    }
